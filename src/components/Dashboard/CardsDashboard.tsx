...
import CardApplyModal from "@/components/Card/CardApplyModal";
...
export default function CardsDashboard() {
    ...
    const balanceEntries = await Promise.all(
      cardList.map(async (card) => {
        ...
        try {
          ...
          if (Number(balanceResponse.code) !== 200 || !balanceResponse.data) {
            return [cardKey, null] as const;
          }
          const balanceValue =
  ...
  const refreshCards = async () => {
    ...
    try {
      ...
      const code = response.code !== undefined ? Number(response.code) : 200;
      if (code !== 200 || !Array.isArray(response.data)) {
    ...
    <div className="space-y-8">
      <header className="flex flex-wrap items-center justify-between gap-4">
        <div className="space-y-2">
          ...
          {errorMessage ? (
            <p className="text-sm text-(--paragraph)">{errorMessage}</p>
          ) : null}
        </div>
        <button
          type="button"
