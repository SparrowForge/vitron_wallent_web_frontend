...
const getCardFee = (bin: BinModel, cardType: "VIRTUAL_CARD" | "PHYSICAL_CARD") =>
...
export default function CardApplyModal({ open, onClose }: CardApplyModalProps) {
  ...
  const supportsPhysical =
    Number(selectedBin?.supportPhysicalCard ?? 0) === 1;
  ...
  const canSubmit =
    ...
    payPassword &&
    (!emailCheck ||
      (verifyType === "email" ? verifyCode : googleCode));
  ...
  useEffect(() => {
    ...
    const loadApplyData = async () => {
      ...
      setErrorMessage("");
      try {
        const [binsResponse, pendingResponse, merchantResponse] =
          await Promise.all([
        ...
        setBins(binsData);
        const firstBinId =
          binsData.length > 0 ? getBinId(binsData[0]) : "";
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      if (Number(response.code) !== 200) {
        setErrorMessage(response.msg || "Apply failed.");
        return;
      }
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-3xl rounded-3xl border border-(--stroke) bg-(--basic-cta) p-6 shadow-2xl"
      ...
      >
        ...
        {errorMessage ? (
          <div className="mt-4 rounded-2xl border border-(--stroke) bg-(--background) px-4 py-2 text-sm text-(--paragraph)">
            {errorMessage}
          </div>
        ) : null}
