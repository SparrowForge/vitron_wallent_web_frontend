...
export default function CardHolderForm({
  ...
  const handleSubmit = async () => {
    ...
    const requiredFields: Array<[keyof typeof form, string]> = [
      ...
      ["accountPurpose", "Account purpose is required."],
      ["expectedMonthlyVolume", "Monthly volume is required."],
      ["countryId", "Country is required."],
      ["state", "State is required."],
      ["city", "City is required."],
      ["address", "Address is required."],
      ["postCode", "Post code is required."],
      ["areaCode", "Area code is required."],
      ["phone", "Phone is required."],
      ["idFrontId", "ID front image is required."],
      ["idHoldId", "Holding ID image is required."],
      ["idNumber", "ID number is required."],
      ["birthday", "Birthday is required."],
      ["startTime", "Issuing date is required."],
      ["endTime", "Expiry date is required."],
    ];
    ...
    if (form.idType !== "PASSPORT" && !form.idBackId) {
      setErrorMessage("ID back image is required.");
      return;
    }
    ...
    try {
      ...
      if (Number(response.code) !== 200) {
        setErrorMessage(response.msg || "Holder submit failed.");
        return;
      }
      setSuccessMessage(
        isEdit
